...
class CanvasWidget {
    ...
    // Automatically adjust the canvas sizes if the view gets resized. This
    // attempts to only do a canvas resize after the user is done adjusting
    // the window.
    ...
    // Set up the dimensions of the canvases.
    _adjustCanvasSizes() {
    ...
    // The "default view" (typically the window of the document).
    get defaultView() {
    ...
    // Get the line width to use for "thin" lines.
    get thinLineWidth() {
        return Math.max(1, Math.floor(this._foreground.width * THIN_LINE_AMT));
    }

    // Get the line width to use for "thick" lines.
    get thickLineWidth() {
