...
export class SliderWidget extends CanvasWidget {
   ...
   * bindings.
   *
   * @param {Element} node DOM Node to insert into.
   * @param {object} [props = null] Initial property values for the widget.
   */
  constructor(node, props = null) {
    super(node, 'slider');
  ...
  renderBackground() {
    ...
    ctx.lineJoin = 'round';
    ctx.beginPath();
    ctx.moveTo(this.#xMin, middleY);
    ctx.lineTo(this.#xMax, middleY);
    ctx.stroke();
  }
  ...
  renderForeground() {
    ...
    const style = this.cloneComputedStyle();
    const ctx = canvas.getContext('2d');
    const height = this.fontSize * 2.5;
    ...
    SliderWidget.#roundRectPath(ctx,
      x - (handleWidth / 2), middleY - (handleHeight / 2),
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(x, middleY - (handleHeight / 2));
    ctx.lineTo(x, middleY + (handleHeight / 2));
    ctx.stroke();

    style.fontSize = this.fontSizePx;
    ctx.font = style.font;
    const boxHeight = this.fontSize * 1.5;
    ...
    ctx.fillText(text, textX, middleY);
  }

  /** @returns {number} The width of the slider handle. */
  get #handleWidth() {
    return this.fontSize * HANDLE_WIDTH_AMT;
  }

  /**
   * @returns {string} Which side the slider is on. Either `left` or `right`.
   */
  get #sliderSide() {
  ...
  /**
   * @returns {number} The current slider x position as an element-relative x
   * coordinate, based on the current value.
   */
  get #sliderX() {
  ...
  /** @returns {string} The current value as formatted text. */
  get #valueText() {
    return this.#value.toFixed(this.#precision);
  }

  /**
   * @returns {number} The element-relative x value corresponding to `minValue`.
   */
  get #xMin() {
  ...
  /**
   * @returns {number} The element-relative x value corresponding to `maxValue`.
   */
  get #xMax() {
    // Note: The actual "throw" of slider is cut off by half a slider width
    // on either end.
   ...
   *
   * @param {number} x The element-relative x coordinate.
   * @returns {number} Corresponding scaled and quantized value.
   */
  #xToValue(x) {
    const totalRange = this.#maxValue - this.#minValue;
    const totalSlide = this.#xMax - this.#xMin;
    const valueRatio = (x - this.#xMin) / totalSlide * this.devicePixelRatio;
