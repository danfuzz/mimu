...
export class SliderWidget extends CanvasWidget {
  ...
  _inputSetup() {
    ...
    const target = this._divNode;  // Wrapper element for this instance.
  ...
  set minValue(value) {
    ...
    this.value = this._value;  // Clamp if necessary.
    this.render();
  }
  ...
  _pullValueFromTarget() {
    if (this._target && this._targetProperty) {
      const v = this._target[this._targetProperty];
      if (v !== undefined) {
        this.value = v;
      }
    }
  ...
  renderForeground() {
    ...
    const width = canvas.width;
    const height = canvas.height;
    const handleWidth = this._handleWidth;
    ...
    const textX = (this._sliderSide === 'left')
      ? (x + (handleWidth * 2))
  ...
  /**
     ...
     */
  get _sliderX() {
    const totalRange = this._maxValue - this._minValue;
    const totalSlide = this._xMax - this._xMin;
    const valueRatio = (this._value - this._minValue) / totalRange;
  ...
  _xToValue(x) {
    const totalRange = this._maxValue - this._minValue;
    const totalSlide = this._xMax - this._xMin;
    const valueRatio = (x - this._xMin) / totalSlide;
    const value = this._minValue + (valueRatio * totalRange);
