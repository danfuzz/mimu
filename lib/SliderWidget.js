...
class SliderWidget extends CanvasWidget {
    ...
    _inputSetup() {
        ...
        // `mousemove` and `touchmove` events could be placed on `target`
        // instead of on `view`, thereby avoiding all the work to convert
        // `view` coordinates to `target` coordinates in the event handlers.

        function touchstart(event) {
            outerThis.value = outerThis._xToValue(event.offsetX);

            view.addEventListener("touchmove", touchmove, true);
            view.addEventListener("touchend", touchend, true);
        }

        function touchend(event) {
            view.removeEventListener("touchmove", touchmove, true);
            view.removeEventListener("touchend", touchend, true);
        }
